...
export class CvHLines{ // extends Canvas3D
  ...
  paint( ): void{ 
    ...
    this.g.strokeStyle = '#7dd3fc';
    this.g.lineWidth = 1.35;
  ...
  drawLine(g: CanvasRenderingContext2D, px1: number, py1: number, px2: number, py2: number): void{
    ...
    lineSegment(g: CanvasRenderingContext2D,  p:Point3D, q:Point3D,
      ...
      for (let i = iStart; i < this.nTria; i++) {
          ...
          // The historical algorithm expected every projected triangle to be
          // counter-clockwise. Normalize it here so faces remain valid when
          // the camera moves to the opposite side of the model.
          if (Tools2D.area2(aScr, bScr, cScr) < 0) {
            const indexTemp = iB; iB = iC; iC = indexTemp;
            const pointTemp = bScr; bScr = cScr; cScr = pointTemp;
          }
          ...
          let a = pol.getA(), b = pol.getB(), c = pol.getC(), h = pol.getH();
          // Use the same plane orientation on both sides of a face. Without
          // this normalization, valid edges could be treated as hidden merely
          // because the polygon winding was reversed.
          if (h > 0) { a = -a; b = -b; c = -c; h = -h; }
          const eps1 = 1e-5 * Math.max(1, Math.abs(h));
          const hP = a * p.x + b * p.y + c * p.z,
